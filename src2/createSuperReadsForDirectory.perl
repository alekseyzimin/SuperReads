...
$cmd = "cat ${joinerOutputPrefix}_* | perl -e '{open(FILE,\$ARGV[0]);while(\$line=<FILE>){chomp(\$line);\@F=split(\" \",\$line);\$sr{\$F[0]}=\$F[1]} while(\$line=<STDIN>){\@l=split(\" \",\$line);if(defined(\$sr{\$l[1]})){print \"\$l[0] \",\$sr{\$l[1]},\" \$l[2] \$l[3]\";}else{print \"\$line\";}}}' $workingDirectory/reduce.tmp >  $reducedReadPlacementFile"; 
