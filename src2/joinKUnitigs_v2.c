...
// Unitig in the overlaps file
    ...
    pclose (infile);
...
#if 1
     FILE *outfile = Fopen ("overlapDataArrayFile.v2.txt", "w");
     int j;
     for (j=0; j<2*overlapCount; j++)
     ...
     fclose (outfile);
     outfile = Fopen ("unitig2OverlapIndex.v2.txt", "w");
     for (j=0; j<2*overlapCount; j++)
	  fprintf (outfile, "%d %d\n", j, unitig2OverlapIndex[j]);
     fclose (outfile);
     outfile = Fopen ("startOverlapByUnitig.v2.txt", "w");
     for (j=0; j<numUnitigs+1+firstUnitigNum; j++)
	  fprintf (outfile, "%d %d\n", j, startOverlapByUnitig[j]);
     fclose (outfile);
     outfile = Fopen ("startOverlapIndexByUnitig2.v2.txt", "w");
     for (j=0; j<numUnitigs+1+firstUnitigNum; j++)
