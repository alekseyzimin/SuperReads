...
export ROOT=`pwd`
export JF_VERSION=`ls -d jellyfish*`
...
(cd CA/src;make)
(cd $JF_VERSION;./configure --prefix=$ROOT; make -j $NUM_THREADS install;)
...
PARAMETERS
#this is k-mer size for deBruijn graph values between 25 and 101 are supported, 35 recommended for high GC (~70%) genomes, and 63 recommended for most genomes with GC% between 40 and 50
GRAPH_KMER_SIZE=35
