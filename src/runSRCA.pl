...
if($generate_default) {
  die "Refusing to overwrite existing configuration file '$config_file'" if(-e $config_file);
  open(FILE, ">", $config_file) or die "Error opening configuration file '$config_file': $!";
  print FILE <<'EOS';
...
sub read_param {
  my ($line) = @_;
  die "Can't parse line '$line'" unless $line =~ /^\s*(\w+)\s*=\s*(.*)$/;
  chomp($2);
  return ($1, $2);
}

open(FILE, "<", $config_file) or die "Can't open config file '$config_file': $!";
while(my $line=<FILE>){
    chomp($line);
    next if($line =~ /^\s*(#|$)/);
    if($line =~ /^DATA\s*$/){
      die("error in config file: mixed PARAMETERS and DATA") if $in_parameters;
      die("duplicate DATA header") if $in_data;
      $in_data = 1;
      next;
    } elsif($line =~ /^PARAMETERS\s*$/){
      die("error in config file: mixed PARAMETERS and DATA") if $in_data;
      die("duplicate PARAMETERS header") if $in_parameters;
      $in_parameters=1;
      next;
    } elsif($line =~ /^PATHS\s*$/) {
      ...
      die("Not in PARAMETERS or DATA section") if(!($in_parameters || $in_data));
    ...
    if($in_parameters==1){
        if($key eq "EXTEND_JUMP_READS"){
            die("bad value for EXTEND_JUMP_READS, enter 0 or 1") unless($param =~ /^[01]$/);
	    $EXTEND_JUMP_READS=int($param);
	} elsif($key eq "DO_HOMOPOLYMER_TRIM"){
            die("bad value for DO_HOMOPOLYMER_TRIM, enter 0 or 1") unless($param =~ /^[01]$/);
	    ...
	    die("bad value for CA_PARAMETERS") if($CA_PARAMETERS eq "");
            ...
            die("bad value for USE_LINKING_MATES, enter 0 or 1") unless $param =~ /^[01]$/;
    ...
    if($in_data==1){
      if($key eq "PE"){
	    my @f=split(" ", $param);
	    die("improper id for PE library $f[0]") if(not(length($f[0])==2));
	    die("duplicate id for PE library $f[0]") if(defined($used_library_ids{$f[0]}));
	    my $pe_info_line ="$f[0] ";
	    $used_library_ids{$f[0]}=1;
	    die("improper mean for PE library $f[0]") if(not(int($f[1])>0));
	    $pe_info_line.="$f[1] ";
	    die("improper stdev for PE library $f[0]") if(not(int($f[2])>0));
	    $pe_info_line.="$f[2] ";
	    die("missing forward file for PE library $f[0]") if(not(-e $f[3]));
	    $pe_info_line.="$f[3] ";
	    if(length($f[4])>0){
		die("missing reverse file for PE library $f[0]") if(not(-e $f[4]));
		$pe_info_line.="$f[4] ";
	    }
	    else{
		$pe_info_line.="$f[3] ";
	    ...
	    my @f=split(" ", $param);
	    die("improper id for JUMP library $f[0]") if(not(length($f[0])==2));
	    die("duplicate id for JUMP library $f[0]") if(defined($used_library_ids{$f[0]}));
	    my $jump_info_line="$f[0] ";
	    $used_library_ids{$f[0]}=1;
	    die("improper mean for JUMP library $f[0]") if(int($f[1])==0);
	    $jump_info_line.="$f[1] ";
	    die("improper stdev for JUMP library $f[0]") unless(int($f[2])>0);
	    $jump_info_line.="$f[2] ";
	    die("missing forward file for JUMP library $f[0]") unless(-e $f[3]);
	    $jump_info_line.="$f[3] ";
	    die("missing reverse file for JUMP library $f[0]") unless(-e $f[4]);
	    $jump_info_line.="$f[4] ";
	    push(@jump_info_array,$jump_info_line);
	}
	elsif($key eq "OTHER"){
	    die("missing or incorrect frg file for OTHER library") unless($param =~/\.frg$/ && (-e $param));
...
if(@prep_ld) {
  unshift(@LD_LIBRARY_PATH, @prep_ld);
}
...
open(FILE,">", $assembly_script) or die "Can't open output file '$assembly_script': $!";
print FILE <<"EOS";
#!/bin/bash

# $assembly_script generated by runSRCA.pl
...
if(not(-e "pe.cor.fa")||$rerun_pe==1){
    ...
    print FILE "\nquorum_error_correct_reads   -p `cat cutoff.txt` --contaminant=$RealBin/../share/adapter_0 -d combined_0 -c 2 -C -m $KMER_COUNT_THRESHOLD -s 1 -g 1 -a $KMER_RELIABLE_THRESHOLD -t $NUM_THREADS -w $WINDOW -e $MAX_ERR_PER_WINDOW $list_pe_files 2>error_correct.log $homo_trim_string | add_missing_mates.pl > pe.cor.fa\n";
    $rerun_pe=1;
}
...
if(scalar(@jump_info_array)>0){
	...
	print FILE "echo -n 'error correct JUMP ';date;\n";
        print FILE "cat combined_0 > /dev/null\n";
	print FILE "\nquorum_error_correct_reads -p `cat cutoff.txt` --contaminant=$RealBin/../share/adapter_0 -d combined_0 -c 2 -C -m $KMER_COUNT_THRESHOLD -s 1 -g 2 -a $KMER_RELIABLE_THRESHOLD -t $NUM_THREADS -w $WINDOW -e $MAX_ERR_PER_WINDOW $list_jump_files 2>error_correct.log $homo_trim_string | add_missing_mates.pl > sj.cor.fa\n";
        $rerun_sj=1;
    }
}
...
#remove all chimeric and all redundant reads from sj.cor.fa
	if(not(-e "sj.cor.clean.rev.fa")||$rerun_pe==1||$rerun_sj==1){
	    ...
	    for($i=0;$i<scalar(@jump_info_array);$i++){
        		my @f=split(/\s+/,$jump_info_array[$i]);
	            	my $if_innie="";
