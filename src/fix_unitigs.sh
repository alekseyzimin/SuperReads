...
perl -e '{open(FILE,"unitig_split_points"); while($line=<FILE>){chomp($line);split(/\s+/,$line);$h{$_[0]}=$_[1];}foreach $v (keys %h){$count1=0;$count2=0;open(FILE,"f_unitig$v");while($line=<FILE>){if($line=~/^data.unitig_coverage_stat/){chomp($line);split(/\s+/,$line); $cov=$_[1];next};next if(not($line =~ /^FRG/)); split(/\s+/,$line);if($_[4] eq $h{$v}){$count2++}elsif($count2==0){$count1++}else{$count2++}}close(FILE);open(FILE,"f_unitig$v");open(OUTFILE,">f_unitig$v.fixed");print OUTFILE "\nunitig $v\nlen 0\ncns\nqlt\ndata.unitig_coverage_stat $cov\ndata.unitig_microhet_prob 1.000000\ndata.unitig_status        X\ndata.unitig_unique_rept   X\ndata.contig_status        U\ndata.num_frags            $count1\ndata.num_unitigs          0\n";$count=-1;while($line=<FILE>){next if(not($line =~ /^FRG/));$count ++;if($count ==$count1){print OUTFILE "\nunitig -1\nlen 0\ncns\nqlt\ndata.unitig_coverage_stat $cov\ndata.unitig_microhet_prob 1.000000\ndata.unitig_status        X\ndata.unitig_unique_rept   X\ndata.contig_status        U\ndata.num_frags            $count2\ndata.num_unitigs          0\n";} print OUTFILE $line;}close(OUTFILE);}}' 
...
chmod 0755 recompute_consensus.sh
./recompute_consensus.sh 1>utgcns.err 2>&1
