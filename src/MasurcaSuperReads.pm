...
sub create_pe_linking_mates {
  ...
  if(@{$config{JUMP_INFO}}){
    print FILE "MAX_LINKING_MATES=`perl -e '{\$g=int('\$ESTIMATED_GENOME_SIZE'/25);\$g=100000000 if(\$g>100000000);print \$g}'`\n";
  } else {
    print FILE "MAX_LINKING_MATES=`perl -e '{\$g=int('\$ESTIMATED_GENOME_SIZE'/2);\$g=100000000 if(\$g>100000000);print \$g}'`\n";
  }
...
EOS
  ...
  print $out "echo -n 'Linking PE reads '; cat ??.linking.frg | grep -c --text '^{FRG' \n";
}
