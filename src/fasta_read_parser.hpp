...
#ifndef __FASTA_READ_PARSER_HPP__
#define __FASTA_READ_PARSER_HPP__

#include <jellyfish/mapped_file.hpp>
#include <jellyfish/concurrent_queues.hpp>
#include <jellyfish/misc.hpp>

class fasta_read_parser {
public:
  static const int max_nb_reads = 100;
  struct read {
    const char *seq_s, *seq_e, *header;
    size_t      hlen;
  };
...
public:
  ...
  fasta_read_parser(int nb_files, char *argv[], unsigned int nb_buffers);
  ...
  class thread {
    fasta_read_parser *parser;
    struct seq        *sequence;
    seq_queue         *rq, *wq;
    int                current_read;

  public:
    thread(fasta_read_parser *_parser) :
      parser(_parser),
